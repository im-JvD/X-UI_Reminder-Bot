...
async def ensure_user_and_check_new(tg_id: int) -> bool:
    """بررسی می‌کند کاربر جدیده یا نه (True یعنی جدید)"""
    async with aiosqlite.connect("data.db") as db:
...
async def start(m: Message):
    # چک عضویت (اطلاع به کاربر می‌دهیم، ولی حتی اگر عضو نبود هم ادامه می‌دهیم تا سوپرادمین مطلع شود)
    try:
    ...
    # فقط بار اول برای سوپر ادمین‌ها
    if is_new:
        user = m.from_user
...
async def report_cmd(m: Message):
    # سوپر ادمین: گزارش کل پنل / ریسلر: فقط اینباندهای خودش
...
async def refresh_report(query):
    ...
    try:
        ...
        # اگر «message is not modified» بود: بی‌صدا نادیده بگیر
        log_error(e)
...
async def send_full_reports():
    ...
    for (tg,) in rows:
        ...
        try:
            ...
            log_error(e)
        async with aiosqlite.connect("data.db") as db:
            await db.execute("INSERT OR REPLACE INTO last_reports(telegram_id, last_json, last_full_report) VALUES (?, ?, ?)",
...
async def check_changes():
    # برای هر ریسلر: پیدا کردن expiring/expired جدید نسبت به last_reports
    async with aiosqlite.connect("data.db") as db:
        rows = await db.execute_fetchall("SELECT DISTINCT telegram_id FROM reseller_inbounds")
    for (tg,) in rows:
        async with aiosqlite.connect("data.db") as db:
